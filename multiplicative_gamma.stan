functions {
  ...
  array[,] matrix P_to_AR(array[] matrix P, matrix Sigma) {
    ...
    Sigma_s[2] = Sigma_s[1]
                 - quad_form_sym(Sigma_star_s[1], phi_splusone[1]');
      ...
      for (splusone in 2 : p) {
        ...
        for (i in 1 : (splusone - 1)) {
          ...
          phi_splusone[i] = phi_s[i]
                            - phi_splusone[splusone]
                              * phi_s_star[splusone - i];
          phi_splusone_star[i] = phi_s_star[i]
                                 - phi_splusone_star[splusone]
                                   * phi_s[splusone - i];
        }
        //stores phi_{s+1}
...
transformed parameters {
  ...
  array[2, p] matrix[m, m] phiGamma; //place to store output of mapping function
